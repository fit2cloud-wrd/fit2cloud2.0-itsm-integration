...
import com.fit2cloud.commons.server.utils.KeycloakLoginUtils;
...
public class LoginController {
    ...
    @Resource
    private KeycloakLoginUtils keycloakLoginUtils;
    @Resource
    ...
    public String ssoLogin(HttpServletRequest request, HttpServletResponse response, Model model) {
        if (keycloakLoginUtils.ssoLoginDisabled()) {
            return loginPath(response, model);
        }
        integrationManager.getITSMService().stashExternalProcessId();
        keycloakLoginUtils.ssoLogin();
        return integrationManager.getITSMService().getFirstPageUrl();
    }
